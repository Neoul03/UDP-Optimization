...
# 260921 Part 3 — 자기정정: bistability 원인 규명 실패 (가설 4건 기각)

## ⚠️ 정정 1: "나쁜 rate(공진)" 가설 기각 (a)
0.5G 단위 정밀 스윕 (44-50G, 각 2회, `logs/ceiling_dimoff_*` 최신):
```
44.0 gb | 45.5 bg | 47.0 gb | 48.5 gb | 50.0 gg
44.5 gb | 46.0 gb | 47.5 bg | 49.0 gg
45.0 gb | 46.5 gg | 48.0 bg | 49.5 bb
```
- **good 15/26 = 58%. 특정 rate가 나쁜 게 아니라 전 구간 동전 던지기.**
- 앞서 "44G 0/3", "48G 0/3"은 **3회 표본의 우연**(p≈0.4³≈6%, 여러 rate 테스트하면 쉽게 발생).
- 두 상태는 뚜렷이 분리: good = rx≈offered/loss 0-0.3%/eff 0.52-0.535, bad = rx 40-44G/loss 7-12%/eff 0.475-0.49.

## ⚠️ 정정 2: "DIM이 원인" 기각 (a)
b=46 고정, 구성별 **10회** (`logs/state_entry_*`):
| 구성 | GOOD |
|---|---|
| DIM off + shed off | 4/10 |
| DIM on + shed off | 4/9 |
| DIM off + shed on | 6/10 |
| DIM on + shed on | 4/10 |
→ **DIM 무관.** 앞서 "DIM off에서 43G 3/3"은 진입률 58%면 확률 20%인 우연. 커밋 `2e7e4d1`의 주장 철회.

## 상태 모델 확정 (a)
- **good state 진입률 ~56%** (누적 20/36), 구성 무관
- **진입하면 안정**: 60s 런 5/7이 46.0G @0% 완주 (`logs/tipping_*`)
- **bad state는 흡수**: 41.3-41.9G 고정, 복구 0회
- **중간 전이는 드묾**: ~45회 중 1회 (t=5s에 46.0@0% → 42.4@7.6%로 전이 후 복구 없음)
- ⇒ "첫 1초에 결정"이 대체로 맞고, 드물게 중간 전이도 존재

## 기각된 가설 4건 (전부 실험 기반)
| 가설 | 검증 | 결과 |
|---|---|---|
| 특정 rate 공진 | 0.5G 단위 26런 | 기각 — 전 구간 58% |
| DIM | 4구성 × 10런 | 기각 — 4/10 vs 4/10 |
| startup 스케줄링 경쟁 | normal/nice-20/chrt-f50 × 10런 | **기각** — 7/10, 6/10, 4/10 (우선순위 무효, FIFO는 악화) |
| 프로세스 메모리 레이아웃 | 서버 상주 × client 5회 × 6 인스턴스 | **기각** — 인스턴스 내 GBGBG로 섞임 (17/29=59%) |

## 남은 후보 (미검증, c)
- flow 시작 시 **타이밍 위상** (첫 버스트 도착과 consumer의 첫 recvmsg 사이 경쟁 — 우선순위가 아니라 순서)
- driver page_pool의 런타임 상태 (warm/cold 페이지 분포)
- → 검증법: **sender rate ramp-up**. 천천히 올리면 초기 적체가 안 생겨 결정적으로 good이어야 함. 맞으면 "UDP에도 slow start가 필요하다"는 실용적 처방이 나옴.

## 측정 인프라 교훈
- `pkill -f 'iperf3 -s'`가 **자기 ssh 셸을 매치해 스스로를 죽임** → `'[i]perf3 -s'` 브라켓 트릭 필수
- 원격 `setsid nohup ... &` 디태치 불안정 → 로컬에서 ssh 백그라운드 유지가 안전

## ⚠️ 정정 3: 버스트 구조(pacing) 가설 기각 (a)
`logs/pacing_*` — `--pacing-timer` 1000/100/20/5 µs (버스트 200배 차이) × 10회:
→ GOOD 5/10, 6/10, 5/10, 6/10. **무관.**
- Limitation: pacing-timer가 실제 송신 버스트를 바꿨는지 직접 검증(패킷 타임스탬프) 안 함. 노브 미작동 가능성 남음 → 이 기각은 다른 넷보다 근거 약함.

## ★★★ 방향 전환: best-case → 기댓값 곡선 (a)
진입률 56%인데 최선값(51.7G)을 인용하는 건 부정직 → rate별 **기댓값**을 측정.
`logs/expected_*` (6회/rate, 1.5MB, DIM off, shed off, flake 제외)

| offered | n | p(good) | **E[goodput]** | good평균 | bad평균 | busy |
|---|---|---|---|---|---|---|
| 46G | 5 | 0.80 | 45.1 | 46.0 | 41.5 | 88% |
| 50G | 6 | 0.50 | 47.0 | 49.8 | 44.3 | 95% |
| **52G** | 6 | 0.00* | **48.5** | — | 48.5 | **100%** |
| 54G | 6 | 0.00 | 46.8 | — | 46.8 | 100% |
| 56G | 6 | 0.00 | 46.0 | — | 46.0 | 100% |
| 58G | 5 | 0.00 | 45.0 | — | 45.0 | 100% |

(*52G 개별: 46.6/50.1/46.7/49.7/47.0/50.9 — loss>1%라 "BAD"로 분류됐으나 **붕괴가 아니라 CPU 포화**)

### ★ 상태 모델 최종 (a)
- **≤42G**: 항상 무손실 (CPU 여유)
- **43-50G**: **bistable** — 무손실(offered 그대로) 또는 붕괴(~41.5G). CPU에 여유가 있는 구간에서만 발생
- **≥52G**: **단일 포화 영역**. busy 100%, goodput 46-51G. bistability 소멸
- ⇒ bistability는 "CPU에 여유가 있는데도 무너지는" 43-50G 구간의 현상. 52G 이상에선 그냥 정상 과부하.

### ★ 정직한 헤드라인 수치
**최대 기댓값 goodput = 48.5 G @ offered 52G, busy 100%** (6/6, 46.6-50.9 범위)
vs **TCP 39.6G** → **+22%**
(best case 51.7G는 "최선 관측치"로만 인용해야 함)
